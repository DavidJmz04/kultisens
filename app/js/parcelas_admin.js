...
function getSondas(usuario) {
    ...
    fetch(url).then(function (respuesta) {
        ...
        //Separa las sondas, de forma que hay una lista de objetos. Dentro de cada objeto estÃ¡ la parcela a la que pertenece la sonda y dos listas: Una con los nombres y otra con los vÃ©rtices:{lat, lng}.

        //Estructura: [{parcela, nombres:[], vertices: [{lat:, lng: }]}, ...]

        sonda = separarSondas(json.datos)

        //Transforma el texto que deberÃ­a ser real a real.
        ...
        crearListaDeSondas(sonda, usuario)
...
//Incluye los datos en el formato buscado en la lista global "sondas". Con ellos tambiÃ©n incluye dos mÃ©todos uno en el que se pone si la marca es visible y otro que la crea.

//Estructura: [{datos: [{parcela, nombres:[], vertices: [{lat:, lng: }]}, ...], visible:, marca:}, ...]

function crearListaDeSondas(json, usuario) {
    ...
    for (var i = 0; i < json.length; i++) {
        ...
        for (var j = 0; j < json[i].vertices.length; j++) {
            ...
            var sonda = {
                ...
                datos: json[i],
                visible: true,
                marca: ponerPuntos([json[i].vertices[j], json[i].nombreSonda[j]], usuario),
            }

            sondas.push(sonda)

            //Como queremos que no aparezca la marca nada mÃ¡s entrar, la establecemos a null.
...
function ponerPuntos(sonda, usuario) {
...
function getParcelas(usuario) {
    ...
    fetch(url)
        ...
        .then(function (json) {
            ...
            //Separa las parcelas, de forma que hay una lista de objetos. Dentro de cada objeto estÃ¡n la id de laparcela, su color, nombre y cultivo, y una lista con los vÃ©rtices:{lat, lng}.
...
function crearListaParcelas(json) {
    ...
    //Incluye los datos en el formato buscado en la lista global "parcelas". Con ellos tambiÃ©n incluye dos mÃ©todos uno en el que se pone si el poligono es visible y otro que lo crea.
